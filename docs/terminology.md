...
**Shape Preservation Constraint**. A shape preservation constraint defines the state of shape formed by a vertex with its adjacents on initialisation. During simulation, a shape preservation constraint will try to mantain this shape with its neighbouring vertices.

...
When Houdini starts up, it evaluates all paths pointed to by the `HOUDINI_PACKAGE_DIR` environment variable to search for packages that are typically defined as JSON files. To allow Houdini to find Adonis, that environment variable must include the path where the `Adonis.json` file is located.

1. Open the System Properties window by searching for *environment variables* in the Windows search bar.
2. Click *Environment Variables* to display the environment variables configured at the system level and for the current user.
3. Click *New...* for the level that you prefer (system or user) and configure `HOUDINI_PACKAGE_DIR` with the path containing the `Adonis.json` file.
4. Adonis will be loaded the next time you launch Houdini.

Depending on your setup, you can instead configure the variable through `houdini.env` by adding:
...
3. Open a new terminal (or run `source ~/.bashrc`) and launch Houdini from that terminal.

For desktop/menu launches, you can instead configure the variable through `houdini.env` by adding:

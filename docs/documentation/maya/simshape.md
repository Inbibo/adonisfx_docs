...
| **Iterations** | Integer | 3 | ✓ | Number of iterations that the solver will execute per simulation step. Greater values mean greater computational cost.<br>- Has a range of \[1, 10\]. Upper limit is soft, higher values can be used. |
...
| **Stiffness Multiplier** | Float | 1.0 | ✓ | Multiplier factor to scale up or down the material stiffness.<br>- Has a range of \[0.0, 2.0\]. Upper limit is soft, higher values can be used. |
...
| **Activation Smoothing** | Integer | 1 | ✗ | Number of iterations for the activation smoothing algorithm. The greater the number, the smoother the activations per patch will be.<br>- Has a range of \[1, 20\]. Upper limit is soft, higher values can be used. |
...
-->
 ...
 - **Muscle Patches File** (String, "", Not animatable): Path to the Adonis Muscle Patches file ([.amp](#generate-muscle-patches)).

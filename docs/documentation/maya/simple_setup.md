...
This page is dedicated to explain, step by step, a simple process of creating and setting every Adonis deformer in Maya. The scenarios presented here will are intended to provide the minimum required configurations to obtain plausible results.
...
<figure>
  ...
  <figcaption>Figure 1: Basic setup for skin simulations.</figcaption>
</figure>
...
Once the skin deformer is properly created it is possible now to paint its weights to correctly setup the deformer properties. To do so, select the simulated mesh and press the ![paint tool](images/adn_paint_tool.png) shelf button or go to AdonisFX Menu > *Paint Tool*.
...
<figure>
  ...
  <figcaption>Figure 6: Basic setup for biceps simulations.</figcaption>
</figure>
...
Pressing play should display the muscle properly attached and with the expected dynamics. However, it still won't respond to external inputs like, for example, flexing an arm which add rigidity to the muscle.
...
<figure>
  ...
  <figcaption>Figure 16: Basic setup for planar biceps simulations.</figcaption>
</figure>

### 1. Creating the deformer

Similar to AdnMuscle, create the deformer by selecting the attachments (arm and forearm joints) and then the mesh to deform (the biceps muscle) and then pressing the ![AdnMuscle](images/adn_ribbon_muscle.png) shelf button or go to AdonisFX Menu > *Ribbon Muscle*.
...
Pressing play should show the muscle properly attached and with the expected dynamics. However, it still won't respond to external inputs like, for example, flexing an arm which add rigidity to the muscle.
...
To tweak additional parameters of the AdnMuscle deformer, the following documentation for [AdnRibbonMuscle](ribbon.md) can be consulted.
...
<figure>
  ...
  <figcaption>Figure 25: Basic setup for facial simulations.</figcaption>
</figure>
...
To add the deformation mesh to the deformer first select the deformation mesh, then the simulated mesh (it is the animation mesh), and then go to AdonisFX Menu > Simshape (edit group) > Add *Deform Mesh*. A message will notify is that the addition of the rest mesh has been done correctly.
...
For this case the Edge Evaluator Node (Information on Adonis Muscle Patches files can be foud in the documentation for [AdnSimshape](simshape.md#muscle-activations)) can be used. To create this node, select the rest mesh, then the deformation mesh, and then go to AdonisFX Menu > Nodes > *Edge Evaluator*.

Once the node is created it will appear in the Node Editor. Connect it to the AdnSimshape deformer. To do this go to AdonisFX Menu > Simshape (edit group) > *Connect Activations Plug*. 
<figure>

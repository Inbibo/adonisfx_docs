...
> - Standalone script execution outside Maya or Houdini requires a separate Python interpreter to be installed.
...
> - Generating ML training data and training models may require additional disk space and memory depending on the dataset and model size. Trained models are generally expected to be under 2GB, but extracted training data may exceed 10GB depending on the scenario.
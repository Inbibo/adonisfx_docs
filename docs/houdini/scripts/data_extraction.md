...
> [!WARNING]
> Before running data extraction, an AdnMLDataProcessing HDA must be configured in the Houdini scene. The `sim_skin_path` and `joints_path` arguments must point to the outputs generated by this processing HDA. The simulated skin should point to the **ADN_OUT_SKIN** output and the joints should point to the **ADN_OUT_ML_JOINTS** output.
>
...
> [!NOTE]
> The `sim_skin_path` and `joints_path` should usually point to Null nodes created after the outputs of the [AdnMLDataProcessing HDA](../tools/ml_data_processing). The simulated skin should point to **ADN_OUT_SKIN** and the joints should point to **ADN_OUT_ML_JOINTS**. These outputs must exist and be valid before calling `data_extraction.extract()`.
...
- The scene contains a valid AdnMLDataProcessing setup, or an equivalent scripted preprocessing setup, producing the simulation skin and joint source used by `sim_skin_path` and `joints_path`.
...
- The `sim_skin_path` and `joints_path` must point to valid outputs generated by an AdnMLDataProcessing HDA, or to equivalent outputs generated by a custom scripted preprocessing setup.

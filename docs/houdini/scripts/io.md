...
The second argument `context` is optional and it is the path to the geometry node where the AdonisFX rig should be gathered from (i.e. */obj/geo1*). If the argument is not provided, the rig will be gathered from the active geometry node in */obj*.

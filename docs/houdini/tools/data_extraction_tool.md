...
4. Set the *Sim Skin Op Path*.
    ...
    Set *Sim Skin Op Path* to the processed simulated skin generated by the [AdnMLDataProcessing HDA](../tools/ml_data_processing). This should usually point to a Null node named **OUT_SKIN**, connected to the **ADN_OUT_SKIN** output plug of the processing HDA.

5. Set the *Joints Op Path*.

    Set *Joints Op Path* to the selected ML joints generated by the [AdnMLDataProcessing HDA](../tools/ml_data_processing). This should usually point to a Null node named **OUT_JOINTS**, connected to the **ADN_OUT_ML_JOINTS** output plug of the processing HDA.
...
<figure style="width:90%; margin-left:5%" markdown>
  ...
  <figcaption><b>Figure 2</b>: Joint list populated from the <i>adnMLJoints</i> point group.</figcaption>
</figure>

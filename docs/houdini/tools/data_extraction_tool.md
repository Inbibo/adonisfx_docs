...
The Data Extraction Tool is implemented in Houdini as a TOP HDA called **AdnMLDataExtraction**. This node is used to extract data for training AdonisML models.
...
<figure style="width:90%;" markdown>
  ...
  <figcaption><b>Figure 1</b>: AdnMLDataExtraction parameter template.</figcaption>
</figure>
...
- **Sim Skin Op Path**: SOP path to the processed simulated skin generated by the [AdnMLDataProcessing HDA](../tools/ml_data_processing). This should usually point to a Null node named **OUT_SKIN**, connected to the **ADN_OUT_SKIN** output plug of the processing HDA.
...
- **Joints Op Path**: SOP path to the processed ML joints generated by the [AdnMLDataProcessing HDA](../tools/ml_data_processing). This should usually point to a Null node named **OUT_JOINTS**, connected to the **ADN_OUT_ML_JOINTS** output plug of the processing HDA. This output should contain the selected ML joints stored in the *adnMLJoints* point group.

- **Set Joint List From Op**: Finds the *adnMLJoints* group from the geometry specified in *Joints Op Path*, then populates and formats the entries in *Joint Names List*.
...
- **Joint Names List**: List of joint names to extract as ML input data. This list is usually populated using *Set Joint List From Op*.
...
- **Record Frame Windows**: Enables the use of specific frame windows for the data extraction process.
...
- **Force Overwrite**: Allows existing extraction data in the output folder to be overwritten.

## Requirements

To use the AdnMLDataExtraction TOP HDA, the data must first be prepared with the [AdnMLDataProcessing HDA](../tools/ml_data_processing).
...
The geometry specified in *Joints Op Path* must contain the selected ML joints in the *adnMLJoints* point group. These joints must also contain the *name* and *transform* point attributes required by the extraction process.

The following optional parameter can also be provided:

- *Muscle Op Path*: SOP path to geometries affected by an **AdnMuscle** or **AdnRibbonMuscle** solver. This is required only when *Extract Muscle Activation Data* is enabled.
...
<figure style="width:90%; margin-left:5%" markdown>
  ...
  <figcaption><b>Figure 2</b>: AdnMLDataExtraction TOP HDA node.</figcaption>
</figure>
...
After cooking the AdnMLDataExtraction TOP HDA, the extraction process writes the data to the folder specified in *Export Folder Path*.

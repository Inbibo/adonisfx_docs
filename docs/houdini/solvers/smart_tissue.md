...
| **ML Activation Smoothing** | Integer | 0       | ✗ | Number of iterations for the activation smoothing algorithm. The greater the number, the smoother the activations will be. Has a range of \[0, 20\]. |
...
| **Out ML Activation Attribute** | String | `adnOutActivation` | ✗ | Name of the point attribute used to write the inferred activation. |
...
| Name | Type | Default | Animatable | Description |
| :--- | :--- | :------ | :--------- | :---------- |
| **ML Activations Attribute**                          | float | 1.0 | ✗ | Specifies the name of the per-point attribute to read activation weights from. The expected attribute name is `adnActivations`. The expected range of the per-point values is \[0.0, 1.0\].  |

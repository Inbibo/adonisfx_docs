...
To be able to activate AdonisFX it is required to purchase a `PRODUCT KEY`. Product keys can be purchased through Inbibo's official [website](https://adonis.inbibo.co.uk/#prices). AdonisFX product keys have the following characteristics:
...
Two ways of activating AdonisFX are supported:

- **Online Activation:** This activation mode requires the user to be connected to the internet. This process allows AdonisFX to connect to the licensing server for validation.
- **Offline Activation:** This activation mode does not require the user to be connected to the internet. Through an activation request file and activation response file, it is possible to activate licenses on machines that do not have direct access to the internet. This activation mode requires the generation of a request file in XML format on the user side, a response file also in XML format on the server side, and ultimately the registration of the response on the user side for the activation process to conclude.
...
Node-Locked Licensing product keys can be purchased through Inbibo's official [website](https://adonis.inbibo.co.uk/#prices).
...
To activate AdonisFX in Online Node-Locked Interactive mode:
  1. Load the plug-in.
  2. A dialog will show up with two options: **Activate** to enter a valid `PRODUCT KEY` in order to enable the full license; **Continue With Trial** to start or continue a 30 day trial period.
...
To be able to activate AdonisFX in interactive mode without relying on internet access an *Activation Request* and an *Activation Response* have to be generated.
...
To activate AdonisFX in Offline Node-Locked Interactive mode:
  ...
  2. Run `OfflineRequest` with admin privileges.
  3. Select **Interactive** mode by entering the value `0`.
  4. Enter a valid product key. An XML file `InteractiveOfflineRequest.xml` will be generated in the same folder.
  ...
  8. Execute `OfflineReponse` (also in `AdonisFX/bin`) with admin privileges.
...
To activate AdonisFX in Online Node-Locked Batch mode:
  ...
  1. Go to `AdonisFX/bin` in the AdonisFX installation folder.
  2. Run `ActivateBatch` with admin privileges.
...
To be able to activate AdonisFX in batch mode without relying on internet access an *Activation Request* and an *Activation Response* have to be generated.
To ease this process several executable files are provided to generate the associated XML files for activation.
...
To activate AdonisFX in Offline Node-Locked Batch mode:
  ...
  4. Enter a valid product key. An XML file `BatchOfflineRequest.xml` will be generated in the same folder.
  5. Send the request file to **adnsupport@inbibo.co.uk** providing enough information to backtrack the source of the activation request.
  ...
  10. The response will be registered and AdonisFX will be ready to be used.s
...
Trial licenses are intended for testing and non-commercial purposes. To use AdonisFX for commercial purposes a `PRODUCT KEY` must be purchased through Inbibo's official [website](https://adonis.inbibo.co.uk/#prices) and activated.

See the AdonisFX [End User License Agreement](https://inbibo.co.uk/adonisfx-maya-eula) for details.
...
The steps for setting up floating licenses are the following:

  1. Locate the server software.
...
To be able to run AdonisFX using floating license these 3 criteria have to be met:
  ...
  1. The floating server is running.
  2. The client machine intended to use AdonisFX has direct access to the floating server.
  3. Configure the environment in the client machines to connect to the server and load AdonisFX.

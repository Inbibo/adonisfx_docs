...
An AdnMuscle requires the following inputs to be provided:
  ...
  - **Targets (T)**: List of objects to which the simulated muscle will interact with. The targets can be either transform nodes (e.g. rig joints, locators, etc) or geometries (e.g. bones). Providing the list of targets is optional on create. However, they are required to add attachments and sliding behavior to the muscle. One or multiple targets are supported.
...
To enable the debugger the *Debug* checkbox must be marked. To select the specific feature to be visualized, choose it from the list provided in *Features*. The features that can be visualized with the debugger in the AdnMuscle deformer are:
 ...
 - **Slide On Geometry**: For each vertex with a slide on geometry weight greater than 0.0, a line will be drawn from the mesh vertex to the closest point on its respective transform target.

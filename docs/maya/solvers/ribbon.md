...
| **Rest Length Multiplier** | 1.0 | Per-vertex weight (`restLengthWeight`) multiplied by the global *Rest Length Multiplier*. Has a range of \[0.0, 1.0\]. A value of 1.0 preserves the global multiplier; lower values reduce the rest lengths locally. Read only at initialization; reinitialize the simulation after editing this map. |

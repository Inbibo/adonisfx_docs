...
To ease with the connection of sensors to the muscle activations AdonisFX provides the **Sensors Connection Editor** tool. Instead of connecting the sensors directly to the muscle deformers, this tool is designed to make the connection through the locators in order to keep the node graph aligned with the standard structure of Sensor -> Locator -> Muscle.

...
5. Press *Load Landmarks* if the scene already contains AdnPointLocator nodes whose names match the provided wildcard patterns.
...
12. Leave the *AdnRadialWrap Deformer (Update Only)* field empty to create a new deformer and automatically connect the input geometry, goal geometry, and defined landmarks.
13. Alternatively, provide the name of an existing deformer in the *AdnRadialWrap Deformer (Update Only)* field to update its landmark connections.
14. Press *Apply*.
